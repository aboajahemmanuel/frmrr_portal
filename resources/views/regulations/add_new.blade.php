                                                ...
                                                <div class="tab-pane active" id="infomation">
                                                    <div class="row gy-4">
                                                        ...
                                                        @endphp
                                                            ...
                                                            <div class="col-md-4">
                                                                <div class="form-group">
                                                                    <label class="form-label" for="market-product-tags">Markets and Productss</label>
                                                                    <div class="form-control-wrap">
  ...
  <div class="col-6">
                                                            <div class="form-group">
                                                                <label class="form-label"
                                                                    for="event-title">Status</label>
                                                                <div class="form-control-wrap">
                                                                    @php
                                                                        // Prepare old values: support single string (comma separated) or array
                                                                        $oldCeased = old('ceased');
                                                                        if (is_string($oldCeased) && strlen($oldCeased) > 0) {
                                                                            $oldCeased = explode(',', $oldCeased);
                                                                        }
                                                                        if (!is_array($oldCeased)) {
                                                                            $oldCeased = [];
                                                                        }
                                                                    @endphp
                                                                    <select class="form-control select2" name="ceased[]"
                                                                        id="ceased-select" multiple="multiple" data-placeholder="Select status(es)">
                                                                        <option value="NULL">N/A</option>
                                                                        @foreach ($statuses as $status)
                                                                            @if (trim($status->name) === 'Active')
                                                                                <option value="Active" {{ in_array('Active', $oldCeased) ? 'selected' : '' }}>
                                                                                    {{ $status->name }}</option>
                                                                            @else
                                                                                <option value="{{ trim($status->name) }}" {{ in_array(trim($status->name), $oldCeased) ? 'selected' : '' }}>
                                                                                    {{ $status->name }}</option>
                                                                            @endif
                                                                        @endforeach
                                                                    </select>
    ...
    <div class="form-control-wrap">
        <select 
            ...
            @foreach ($relatedDocuments as $docs)
                <option value="{{ $docs->id }}">{{ $docs->title }} ({{ $docs->month->name }} - {{ $docs->year->name }})</option>
            @endforeach
        </select>
    </div>
</div>
    ...
    <div class="form-group">
        ...
        <div class="form-control-wrap">
            <select 
                ...
                @foreach ($nestedRelatedDocuments as $docs)
                    <option value="{{ $docs->id }}">{{ $docs->title }} ({{ $docs->month->name }} - {{ $docs->year->name }})</option>
                @endforeach
            </select>
        </div>
