...
!function (NioApp, $) {
      ...
      $wizard.each(function () {
        ...
        $self_id.steps({
          ...
          onFinished: function onFinished(event, currentIndex) {
            document.getElementById("wizard-01").submit();
          }
        }).validate({
          errorElement: "span",
          errorClass: "invalid",
  ...
  NioApp.DataTable.init = function () {
    NioApp.DataTable('.datatable-init:not(#regulations-table):not(.no-dt)', {
      responsive: {
        details: true
      }
    });
    NioApp.DataTable('.datatable-init-export:not(#regulations-table):not(.no-dt)', {
      responsive: {
        details: true
      },
